...
import com.mapbox.navigation.core.trip.session.RoadNameObserver
import com.mapbox.navigation.tripdata.maneuver.api.MapboxManeuverApi
import com.mapbox.navigation.tripdata.maneuver.api.MapboxRoadNameApi
...
class MapboxNavigationView(private val context: ThemedReactContext) :
  ...
  /**
   * Generates updates for the [MapboxRoadNameView] to display the current road name.
   */
  private var roadNameApi: MapboxRoadNameApi? = null
  ...
  /** Observes when the current road name changes. */
  private val roadNameObserver = RoadNameObserver { roadName ->
    roadNameApi?.let { api ->
        val updatedRoadName = api.onRoadNameChanged(roadName)
        binding.roadNameView.visibility = if (updatedRoadName.isValid()) View.VISIBLE else View.GONE
        binding.roadNameView.render(updatedRoadName)
    }
  }
  ...
  /** Aplica cores de texto pretas em todos os TextViews dentro do tripProgressView */
  private fun applyTripProgressTextColors() {
    try {
      binding.tripProgressView?.let { view ->
        ...
        fun findTextViews(parent: android.view.ViewGroup) {
        ...
        if (view is android.view.ViewGroup) {
          ...
          if (textViews.isNotEmpty()) {
            Log.d(
                    ...
                    "✅ Aplicadas cores pretas em ${textViews.size} TextViews do tripProgressView"
            )
          }
        }
      }
  ...
  private fun initNavigation() {
    ...
    try {
      ...
      // initialize road name api
      roadNameApi = MapboxRoadNameApi()
  ...
  private fun startRoute() {
    ...
    mapboxNavigation?.registerRoadNameObserver(roadNameObserver)
  ...
  override fun onDetachedFromWindow() {
    ...
    mapboxNavigation?.unregisterRoadNameObserver(roadNameObserver)
    ...
    try {
      val features =
              radarsToDisplay.map { radar: RadarPoint ->
                ...
                // O usuário reportou: Placas em 0.0 estão certas, outros em 0.0 estão para baixo.
                // Logo: Placas = 0.0, Outros = 180.0 (para inverter o que está para baixo).
