...
/** Ponto de radar para exibição no mapa (id, coordenadas, limite de velocidade, tipo). */
data class RadarPoint(
        val id: String,
        val latitude: Double,
        val longitude: Double,
        val speedLimit: Double? = null,
        val type: String? = null // Tipo do radar: "semaforico", "fixo", "móvel", etc.
)
...
class MapboxNavigationView(private val context: ThemedReactContext) :
  ...
  /*
   * Padding da câmera: estilo Waze — modal fixo embaixo, posição do usuário jogada para cima.
   * Bottom = altura do modal (trip progress + speed + margens); top/laterais mínimos.
   * Assim a câmera mantém o usuário na área visível acima do modal, sem sobreposição.
   */
  ...
  // Following: usuário na área acima do modal (~220dp para acomodar o floating card + margens)
  ...
  private var filteredRadars: List<RadarPoint> = listOf() // Radares filtrados próximos à rota
  ...
  /** Opções de formatação de distância (usadas também pelo Speed Limit API). */
  ...
  private val offRouteObserver =
          com.mapbox.navigation.core.trip.session.OffRouteObserver { offRoute ->
            if (offRoute) {
              Log.d("MapboxNavigationView", "🚗 Usuário saiu da rota")
  ...
  private val locationObserver =
          object : LocationObserver {
            ...
            override fun onNewLocationMatcherResult(locationMatcherResult: LocationMatcherResult) {
              ...
              // Speed: sempre visível — limite da via (SpeedInfoView) ou velocidade atual
              // (TextView)
  ...
  private val routeProgressObserver = RouteProgressObserver { routeProgress ->
    // Atualizar rota atual se necessário (pode mudar durante navegação)
      ...
      if (maneuverApi != null && binding.maneuverView != null) {
        ...
        maneuvers.fold(
                { error -> Log.w("Maneuvers error:", error.throwable) },
                {
                  ...
                  // Mostrar instrução de manobra (layout tem maneuverView)
    ...
    // update trip progress: duas linhas (tempo em destaque + distância — ETA), estilo primeira
    // imagem
  ...
  private fun updateTripProgressUi(routeProgress: RouteProgress) {
    try {
      // Verificar se tripProgressApi está inicializado e tripProgressView não é null
      // O MapboxTripProgressView atualiza automaticamente via RouteProgressObserver
      // Não é necessário atualizar manualmente - a view já está conectada ao observer
      if (tripProgressApi != null && binding.tripProgressView != null) {
        ...
        // Aplicar cores de texto pretas após a view ser atualizada
  ...
  /** Aplica cores de texto pretas em todos os TextViews dentro do tripProgressView com suporte a recursão profunda */
  private fun applyTripProgressTextColors() {
    try {
      binding.tripProgressView?.let { view ->
        view.setBackgroundColor(android.graphics.Color.WHITE)
        val primaryColor = android.graphics.Color.parseColor("#212121")
        val secondaryColor = android.graphics.Color.parseColor("#666666")
        
        fun findAndStyle(v: android.view.View, isSecondary: Boolean = false) {
            if (v is TextView) {
                // Forçar cor e estilo via código diretamente para evitar problemas de tema
                v.setTextColor(if (isSecondary) secondaryColor else primaryColor)
                v.setShadowLayer(0f, 0f, 0f, 0) // Remover sombras se houver
                
                if (!isSecondary) {
                  v.typeface = android.graphics.Typeface.DEFAULT_BOLD
                  v.textSize = 18f
                } else {
                  v.textSize = 14f
                }
                
                // Tentar aplicar o estilo do resources também
                try {
                  if (isSecondary) {
                    TextViewCompat.setTextAppearance(v, R.style.TripProgressSecondaryTextAppearance)
                  } else {
                    TextViewCompat.setTextAppearance(v, R.style.TripProgressPrimaryTextAppearance)
                  }
                } catch (e: Exception) {}
  ...
  private val routesObserver = RoutesObserver { routeUpdateResult ->
    try {
      ...
      if (routeUpdateResult.navigationRoutes.isNotEmpty()) {
        ...
        // Filtrar radares próximos à rota
        filterRadarsNearRoute()
  ...
  private fun initNavigation() {
    ...
    try {
      ...
      Log.d("MapboxNavigationView", "Iniciando navegação: origin=$origin, destination=$destination")
      ...
      // Aplicar cores de texto após a view ser criada
      binding.tripProgressView?.postDelayed(
              { applyTripProgressTextColors() },
              500
      ) // Delay para garantir que a view está totalmente renderizada
      ...
      try {
        ...
        Log.w("MapboxNavigationView", "Voice instructions não disponíveis: ${e.message}")
      ...
      binding.mapView.mapboxMap.loadStyle(NavigationStyles.NAVIGATION_DAY_STYLE) { style ->
        ...
        // Callouts padrão do SDK: bolha com duração/ETA na rota
        try {
        ...
        try {
          ...
          // Atualizar radares se já houver algum definido
        ...
        // Compass: posição abaixo dos controles (recenter/overview/sound), canto inferior direito
        try {
        ...
        binding.mapView.mapboxMap.addOnMapClickListener { point ->
          try {
              ...
              if (expected.isValue) {
                ...
                if (features != null && features.isNotEmpty()) {
                   ...
                   if (routes != null && routes.size > 1) {
                      // Melhor: Pegar a rota de índice 1 (primeira alternativa) e definir como primária.
            ...
            // Retornar false para permitir que o mapa processe outros clicks se necessário
      ...
      Log.e("MapboxNavigationView", "Erro ao inicializar navegação", e)
      sendErrorToReact("Erro ao iniciar navegação: ${e.message}")
    }
  }
  ...
  private fun findRoute(coordinates: List<Point>) {
    ...
    mapboxNavigation?.requestRoutes(
            RouteOptions.builder()
                    ...
                    ) // Necessário para speedLimitInfo
                    .build(),
            object : NavigationRouterCallback {
              ...
              override fun onRoutesReady(
                ...
                // Armazenar rotas alternativas (até 3 melhores)
                alternativeRoutes = routes.take(3)
                currentRouteIndex = 0

                // Mostrar rotas alternativas no mapa antes de iniciar navegação
  ...
  private fun setRouteAndStartNavigation(routes: List<NavigationRoute>) {
    ...
    try {
      ...
      Log.e("MapboxNavigationView", "Erro ao iniciar sessão de navegação", e)
      sendErrorToReact("Erro ao iniciar navegação: ${e.message}")
    }
  }
  ...
  private fun startRoute() {
    ...
    if (coordinatesList.size < 2) {
      Log.e(
              ...
              "startRoute: origem e destino são obrigatórios (pontos=${coordinatesList.size})"
      )
      sendErrorToReact("Origem e destino são obrigatórios")
      return
    }
  ...
  fun setBottomPadding(padding: Double) {
    try {
      ...
      updateCameraPadding()
      Log.d("MapboxNavigationView", "⬇️ Bottom padding atualizado para: $padding")
    } catch (e: Exception) {
  ...
  fun setRadars(radars: List<RadarPoint>) {
    try {
      this.radars = radars
      Log.d("MapboxNavigationView", "📍 ${radars.size} radares definidos")

      // Filtrar radares se houver rota ativa
      if (currentRoute != null) {
        filterRadarsNearRoute()
        updateRadarsOnMap() // ✅ Atualizar mapa IMEDIATAMENTE após filtrar
      } else {
        filteredRadars = radars
        updateRadarsOnMap() // Atualizar mapa quando não há rota
      }
  ...
  fun setNearbyRadarIds(radarIds: Set<String>) {
    try {
      ...
      Log.d("MapboxNavigationView", "📡 ${radarIds.size} radares próximos definidos")
      updateRadarAnimations()
    } catch (e: Exception) {
      Log.e("MapboxNavigationView", "Erro ao definir radares próximos", e)
    }
  }
  ...
  private fun updateCameraPadding() {
    try {
      ...
      // Atualizar padding da câmera baseado no additionalBottomPadding
      // Usar lógica diferencial para evitar acúmulo infinito de padding
      val currentPadding = viewportDataSource.followingPadding
      
      // Aplicar densidade se necessário (assumindo que entrada é DP)
      // Mas para manter compatibilidade com comportamento atual (que usuário disse "sobe um pouco"),
      ...
      // Vamos assumir que o valor passado já está ok ou ajustar depois se precisar.
      
      val paddingToAdd = additionalBottomPadding * pixelDensity // Convertendo para pixels para garantir efeito correto

      // Recuperar base subtraindo o último aplicado
      ...
      // Atualizar último valor aplicado
      lastAppliedAdditionalPadding = paddingToAdd

      Log.d("MapboxNavigationView", "✅ Camera padding atualizado: bottom=$newBottomPadding (added: $paddingToAdd)")
    } catch (e: Exception) {
    ...
    try {
      val imageNames =
              listOf(
                      ...
                      "radar_semaforico", // Radar semafórico
                      "radarSemaforico", // Variação de nome
                      "radarMovel",       // Radar móvel
                      "radar_movel",      // Variação de nome
                      "radarmovel"       // Variação de nome
              )
      ...
      imageNames.forEach { imageName ->
        try {
          ...
          // Se não encontrar, tentar tudo minúsculo (comum no Android)
          if (resourceId == 0) {
          ...
          if (resourceId == 0) {
            Log.w("MapboxNavigationView", "⚠️ Recurso não encontrado: assets_images_$imageName")
            return@forEach
          }
          ...
          if (!imageExists) {
            ...
            Log.d("MapboxNavigationView", "✅ Imagem $imageName carregada")
          }
        } catch (e: Exception) {
          ...
          if (resourceId != 0) {
            ...
            if (bitmap != null) {
              ...
              if (!imageExists) {
                ...
                Log.d("MapboxNavigationView", "✅ Imagem faltante $missingId carregada via listener")
              }
            }
          }
        } catch (e: Exception) {
  ...
  /** Filtra radares próximos à rota atual Usa distância perpendicular máxima de 100 metros */
  private fun filterRadarsNearRoute() {
    ...
    try {
      ...
      if (directionsRoute == null) {
        Log.w("MapboxNavigationView", "⚠️ directionsRoute é null, mostrando todos os radares")
        filteredRadars = radars
        return
      }
      ...
      if (routeGeometry == null) {
        Log.w("MapboxNavigationView", "⚠️ routeGeometry é null, mostrando todos os radares")
        filteredRadars = radars
        return
      }
      ...
      try {
        ...
        if (lineString == null) {
          Log.w(
                  "MapboxNavigationView",
                  "⚠️ Não foi possível converter routeGeometry para LineString"
          )
          filteredRadars = radars
          return
        }
        ...
        if (routeCoordinates.isEmpty()) {
          Log.w(
                  "MapboxNavigationView",
                  "⚠️ routeCoordinates está vazio, mostrando todos os radares"
          )
          filteredRadars = radars
          return
        }
        ...
        // Filtrar radares próximos à rota (distância perpendicular < 100m)
        ...
        Log.d(
                "MapboxNavigationView",
                "🔍 Filtrados ${filteredRadars.size} de ${radars.size} radares próximos à rota"
        )

        // Atualizar radares no mapa após filtrar
        updateRadarsOnMap()
      } catch (e: Exception) {
      ...
      Log.e("MapboxNavigationView", "Erro ao filtrar radares próximos à rota", e)
      filteredRadars = radars // Fallback: mostrar todos os radares
      updateRadarsOnMap()
    }
  }

  /** Calcula a distância mínima perpendicular de um ponto até a rota Retorna distância em metros */
  ...
  /**
   * Calcula distância perpendicular de um ponto até um segmento de linha Usa fórmula de distância
   * ponto-linha Retorna distância em metros usando fórmula de Haversine
   */
  private fun calculatePerpendicularDistance(
    ...
    // Calcular produto escalar para encontrar ponto mais próximo no segmento
    val t =
    ...
    // Calcular distância Haversine entre ponto e ponto mais próximo
  ...
  private fun updateRadarsOnMap() {
    ...
    // Durante navegação: usar radares disponíveis imediatamente (não esperar filteredRadars)
    // Isso garante que radares apareçam instantaneamente
    // ESTABILIDADE: Ordenar por ID para evitar "pisca-pisca" quando a lista muda de ordem
    val radarsToDisplay = if (currentRoute != null && filteredRadars.isNotEmpty()) {
      // Durante navegação: mostrar radares próximos à rota (limitado para performance)
      filteredRadars.take(500)
    } else {
      // Fora de navegação ou se filtro falhar: limitar a 1000 radares
    ...
    try {
      val features =
              radarsToDisplay.map { radar: RadarPoint ->
                ...
                val radarType = radar.type?.lowercase()?.replace("á", "a")?.replace("é", "e")?.replace("í", "i")?.replace("ó", "o")?.replace("ú", "u") ?: ""
                
                // Determinar ícone, tamanho e ROTAÇÃO baseado no tipo
                // Todos os ícones agora usam rotação 0.0 pois os assets já estão na orientação correta.
                // Placas e radares aparecem apontando para cima por padrão.
                val (iconImage, iconSize, iconRotate) = when {
                  radarType.contains("semaforo") -> Triple("radarSemaforico", 0.20, 0.0)
                  radarType.contains("movel") || radarType.contains("mobile") -> Triple("radarMovel", 0.20, 0.0)
                  radarType.contains("fixo") || radarType.contains("placa") -> {
                    ...
                    // Se for fixo mas não tiver velocidade, ainda assim usar ícone de radar fixo/placa (default 60 ou genérico)
                    val icon = if (closestSpeed > 0) "placa$closestSpeed" else "radar"
                    Triple(icon, 0.24, 0.0)
                  }
                  radarType.contains("lombada") -> Triple("radar", 0.20, 0.0)
                  radarType.contains("policia") -> Triple("radar", 0.20, 0.0)
                  radarType.contains("reportado") -> Triple("radar", 0.20, 0.0)
                  else -> {
                    // Padrão: tentar usar velocidade se disponível, senão radar genérico
                    val speeds = listOf(20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160)
                    val closestSpeed =
                            if (speedLimit > 0) {
                              speeds.minByOrNull { kotlin.math.abs(it - speedLimit) } ?: 0
                    ...
                    if (closestSpeed > 0) {
                        Triple("placa$closestSpeed", 0.24, 0.0)
                    } else {
                        Triple("radar", 0.20, 0.0)
                    }
                  }
                }

                // Log apenas dos primeiros radares para não poluir
                if (radarsToDisplay.indexOf(radar) < 5) {
                    Log.d("MapboxNavigationView", "📍 Mapeado: id=${radar.id}, type=${radar.type}, icon=$iconImage, speed=$speedLimit")
                }
                ...
                Feature.fromGeometry(
                        ...
                        JsonObject().apply {
                          ...
                          addProperty("iconSize", iconSize)  // Tamanho dinâmico
                          addProperty("iconRotate", iconRotate) // Rotação dinâmica
      ...
      if (style.styleSourceExists("radars-source")) {
           ...
           dataValue.value?.let { value ->
               ...
               if (result.isError) {
                   Log.e("MapboxNavigationView", "❌ Erro ao atualizar dados da source: ${result.error}")
               } else {
                   Log.d("MapboxNavigationView", "✅ Dados de radares atualizados via setStyleSourceProperty")
               }
           } ?: Log.w("MapboxNavigationView", "⚠️ dataValue.value é null, pulando atualização de radares")
      } else {
          ...
          when (val value = sourceValueResult.value) {
            null -> {
              ...
              Log.d("MapboxNavigationView", "✅ Source 'radars-source' criada inicialmente")
              
              // Garantir images
              loadRadarImages(style)
              
              // Criar layerapenas se não existir
              if (!style.styleLayerExists("radars-layer-symbol")) {
                try {
                    ...
                    layerValueResult.value?.let { value ->
                        ...
                        Log.d("MapboxNavigationView", "✅ Layer 'radars-layer-symbol' adicionado")
                    } ?: Log.w("MapboxNavigationView", "⚠️ layerValueResult.value é null, layer não criado")
                } catch (e: Exception) {
                    Log.e("MapboxNavigationView", "❌ Erro ao criar layer", e)
                }
              }
            }
          }
  ...
  private fun updateNearbyRadarsInSource() {
    // Reutilizar lógica centralizada do updateRadarsOnMap
    // Isso garante consistência e evita duplicação de código
    updateRadarsOnMap()
  }

  /** Mostra rotas alternativas no mapa com callouts e informações */
  private fun showAlternativeRoutes(routes: List<NavigationRoute>) {
    val style = getCurrentStyle() ?: return

    try {
      // Criar features para cada rota alternativa com informações
      ...
      routes.forEachIndexed { index, route ->
        ...
        // Criar callout no ponto médio da rota
        ...
        null -> {
          ...
          when (val layerValue = layerValueResult.value) {
            null -> {
              ...
              Log.d(
                      "MapboxNavigationView",
                      "✅ Rotas alternativas exibidas: ${alternativeRouteFeatures.size} rotas"
              )
            }
          }
        }
  ...
  /**
   * Promove uma rota alternativa para ser a rota principal. A seleção é feita tocando no callout da
  ...
  private fun selectAlternativeRoute(selectedIndex: Int, source: String) {
    ...
    try {
      ...
      Log.d(
              "MapboxNavigationView",
              "✅ Rota alternativa selecionada: index=$selectedIndex (source=$source)"
      )
  ...
  /**
   * Atualiza a visualização das rotas após uma rota alternativa ser selecionada como principal
   */
  private fun updateRouteVisualizationAfterSelection(selectedIndex: Int) {
    try {
      // Atualiza a aparência das rotas para mostrar qual é a principal
      Log.d("MapboxNavigationView", "🔄 Atualizando visualização após seleção da rota: $selectedIndex")
      
      // Aqui poderíamos atualizar a opacidade ou cor das rotas alternativas
      // para destacar a rota principal
      
      // Por exemplo, poderíamos alterar a espessura ou cor da rota principal
      // em relação às demais rotas alternativas
      
      // Atualizar também os callouts para refletir a nova ordem
      val style = binding.mapView.mapboxMap.style
      if (style != null) {
      ...
      Log.e("MapboxNavigationView", "Erro ao atualizar visualização após seleção", e)
    }
  }
  
  private fun setupRouteAndCalloutClickListener() {
    // Adiciona listener de clique no mapa para permitir seleção de rota alternativa
    binding.mapView.getMapboxMap().addOnMapClickListener { point ->
      try {
        Log.d("MapboxNavigationView", "🖱️ Mapa clicado - verificando se é clique em rota alternativa")
        
        // A funcionalidade de seleção de rota por clique está implementada
        // Apenas alternamos entre as rotas disponíveis quando o usuário clica no mapa
        if (alternativeRoutes.size > 1) {
          // Seleciona a próxima rota na lista
          val currentIndex = currentRouteIndex
          val nextIndex = if (currentIndex < alternativeRoutes.size - 1) currentIndex + 1 else 0
          
          Log.d("MapboxNavigationView", "🔄 Alternando rota: $currentIndex -> $nextIndex")
      ...
      false // Permitir que outros listeners processem o clique também
    }
  }
