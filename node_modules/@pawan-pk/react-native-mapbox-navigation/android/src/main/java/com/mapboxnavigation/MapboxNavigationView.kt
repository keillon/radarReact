...
class MapboxNavigationView(private val context: ThemedReactContext) :
  ...
  private fun applyTripProgressTextColors() {
    try {
      binding.tripProgressView?.let { view ->
        // O MapboxTripProgressView contém vários TextViews internos. 
        // Vamos aplicar os estilos programaticamente para garantir visibilidade.
        ...
        fun findAndStyle(v: android.view.View, isSecondary: Boolean = false) {
            if (v is TextView) {
                ...
                for (i in 0 until v.childCount) {
                    // O primeiro texto costuma ser o tempo restante (Primary)
                    // Os demais (distância, ETA) costumam ser secundários
  ...
  private fun updateRadarsOnMap() {
    ...
    val radarsToDisplay = if (currentRoute != null) {
      // Durante navegação: pegar primeiros 1000 radares (aumentado de 300)
      radars.sortedBy { it.id }.take(1000)
    } else {
      // Fora de navegação: limitar a 1000 radares para performance
      radars.sortedBy { it.id }.take(1000)
    }
      ...
      val features =
              radarsToDisplay.map { radar: RadarPoint ->
                ...
                val (iconImage, iconSize, iconRotate) = when {
                  radarType.contains("semaforo") -> Triple("radarSemaforico", 0.05, 0.0)
                  radarType.contains("movel") || radarType.contains("mobile") -> Triple("radarMovel", 0.05, 0.0)
                  radarType.contains("fixo") || radarType.contains("placa") -> {
                    ...
                    Triple(icon, 0.15, 0.0)
                  }
                  radarType.contains("lombada") -> Triple("radar", 0.06, 0.0)
                  radarType.contains("policia") -> Triple("radar", 0.09, 0.0)
                  radarType.contains("reportado") -> Triple("radar", 0.05, 0.0)
                  else -> {
